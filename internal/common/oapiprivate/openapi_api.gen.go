...
type EndGame200Response struct {
}

func (response EndGame200Response) VisitEndGameResponse(w http.ResponseWriter) error {
	w.WriteHeader(200)
	return nil
}

type EndGamedefaultJSONResponse struct {
	Body       Error
	StatusCode int
}
...
type AddGameStepsResponseObject interface {
	VisitAddGameStepsResponse(w http.ResponseWriter) error
}

type AddGameSteps201JSONResponse struct {
	GameSteps []GameStep `json:"game_steps"`
}

func (response AddGameSteps201JSONResponse) VisitAddGameStepsResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(201)
...
type AddGameStepsdefaultJSONResponse struct {
	Body       Error
	StatusCode int
}
...
// EndGame operation middleware
func (sh *strictHandler) EndGame(w http.ResponseWriter, r *http.Request, gameId string) {
	var request EndGameRequestObject

	request.GameId = gameId

	var body EndGameJSONRequestBody
	if err := json.NewDecoder(r.Body).Decode(&body); err != nil {
		sh.options.RequestErrorHandlerFunc(w, r, fmt.Errorf("can't decode JSON body: %w", err))
		return
	}
...
func (sh *strictHandler) AddGameSteps(w http.ResponseWriter, r *http.Request, gameId string) {
	...
	handler := func(ctx context.Context, w http.ResponseWriter, r *http.Request, request interface{}) (interface{}, error) {
		return sh.ssi.AddGameSteps(ctx, request.(AddGameStepsRequestObject))
	}
	for _, middleware := range sh.middlewares {
		handler = middleware(handler, "AddGameSteps")
	}
